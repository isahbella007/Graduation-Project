...
if(isset($_GET["cid"])){
 ...
 $_SESSION['orderid'] = $orderid;
  ...
  $check='val';
  $sql1= "UPDATE messages  SET status = 'SEEN'  WHERE FromUser = '$cid' ;";
...
$sql1 = "SELECT * FROM messages  WHERE (FromUser = '$restid' AND ToUser ='$cid' AND trim(orderid) ='$orderid'  ) OR (FromUser = '$cid' AND ToUser ='$restid' AND trim(orderid) ='$orderid' )  ;";
  $result1 = mysqli_query($conn,$sql1);
  $resultcheck1 = mysqli_num_rows($result1);
  while ($row1 = mysqli_fetch_assoc($result1)){
    if($row1['FromUser']== $restid){
      echo"
...
<div class="modal-footer">
  <form action="insertmessage.php" method="post">
    ...
    <input type="text" value=' <?php echo $restid; ?>' name ="fromuser" id="fromuser" hidden/>
        <input type="text" value='<?php echo $cid; ?> ' name="touser" id="touser" hidden/>
        ...
        <br>
  <button id="send" name="rsend" class="btn btn-primary" style="height: 70%;"> send </button>
  </form>
</div>
</div>
</div>
